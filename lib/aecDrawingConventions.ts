...
CONSTRUCTION ASSEMBLY / FRAMING CALLOUTS relevant to ceiling height, duct routing, and insulation:
- Insulation callouts typically read as "R-##" followed by a method (BATT, BLOWN, SPRAY FOAM, RIGID) and sometimes a location (WALLS, CEILING, ATTIC, FLOOR) - e.g. "R-19 BATT INSULATION" (almost always a wall), "R-38 BLOWN INSULATION" (almost always a ceiling/attic). The method word alone is often enough to infer location even without an explicit one - blown/loose-fill insulation is applied to attics/ceilings, not walls.
- Ceiling height appears in different forms depending on drawing type: a floor plan often gives a flat "X' CEILING" / "X' CLG" note per room; a wall/building section gives it as an elevation difference (e.g. "FIN. FLOOR ELEV: 0'-0"" up to "CEIL'G HT. ELEV: +10'-0""); a roof/framing plan may show only a pitch (e.g. "8:12 PITCH"), which by itself does NOT tell you ceiling height at any given point - don't compute one from a bare pitch note.
- A "KNEE WALL" callout (e.g. "6' KNEE WALL") marks where a sloped/attic ceiling meets a short vertical wall - a room described this way (common in bonus rooms and some upper-level rooms) has non-uniform ceiling height across its footprint. Report that room's main/usable ceiling height, not the lowest point at the knee wall.
- A beam/header callout ("8"x12" WOOD BEAM", "2 PLY 2X10", "LVL BEAM", "ENGINEERED LAM BEAM") marks a structural member that can be a dropped, duct-routing-relevant obstruction in that space - relevant only to reasoning about duct_location per the schema below, never to report on as structural fact (see the out-of-scope note above).

SCHEDULE TABLES (window schedules, door schedules, and similar) - see also this task's own separate, blanket-distrust handling of window_schedule transcription further down; the conventions below are about correctly READING a schedule's structure, not about how much to trust a transcription of one:
- A schedule's MARK column (a letter or number) correlates to a small tagged symbol drawn on the floor plan next to each actual opening - the schedule row itself does not say WHERE each instance is; only a matching tag on the floor plan does. A schedule read in isolation tells you what sizes/types exist in the building, not which room has which one.
- A schedule's QTY column is a total count across the whole project/sheet set, not a per-room count - "QTY 4" next to one window type means 4 total instances somewhere in the building, not 4 in every room that has a window of that type.
- Size format in a schedule is normally WIDTH x HEIGHT (e.g. "3'0" X 5'0""), giving the rough opening or unit size - a reasonable basis for window area when nothing more precise exists, but treat it as an approximation of glass area, not an exact measurement.

MECHANICAL (HVAC) PLAN CONVENTIONS:
- Supply air outlets (registers/diffusers) are commonly a small rectangle or square with diagonal lines or an X through it; return air openings (grilles) are often a similar but distinguishable symbol. A legend on the mechanical sheet itself, when present, is the authoritative source for what a given symbol means in THIS set - don't assume the same exact symbol convention holds across every drawing set.
- Ductwork is normally drawn single-line (one line for the run's centerline, with a printed width/diameter alongside it, e.g. "12x8" or "10""), not double-line/to-scale - never infer a duct's physical size from how thick its line is drawn; read the printed dimension.
- Equipment tags (e.g. "AHU-1", "RTU-2", "CU-1") identify a specific piece of equipment referenced elsewhere - an equipment schedule, a manufacturer cut sheet, or a legend. The tag alone never gives capacity or model; look for what it references before treating it as a complete fact.
- A thermostat is typically a small circle or square symbol with a "T" inside it, or explicitly labeled "THERMOSTAT" - its plan location indicates which zone/system it controls when zones aren't otherwise labeled.

STRUCTURAL SHEET LITERACY - for recognizing what to skip (see the out-of-scope rule above), not for reasoning about structure itself:
- A structural sheet typically shows a framing plan (joist/rafter/truss layout with a direction arrow and spacing callout, e.g. "2X8 JOISTS @ 16" O.C."), a foundation plan, a beam/column schedule, or a block of structural design-criteria notes (live/dead load, wind/seismic criteria) - all structural design content, out of scope to report on or compute.
- A blanket disclaimer block on a structural (or any) sheet, e.g. "A LICENSED STRUCTURAL ENGINEER SHOULD REVIEW..." or "THESE PLANS ARE DESIGNED FOR NON-EXPANSIVE SOILS," is a legal/liability note about the design process itself, not a project fact - never extract it as if it were a building characteristic.
- Still worth reading a structural sheet for incidental HVAC-relevant facts even though its primary content is out of scope: a joist depth/spacing note can bound how much vertical space exists for duct runs between floors; a beam callout can mark an obstruction a duct has to route around.

ELECTRICAL SHEET LITERACY - same purpose as structural above; electrical design itself is out of scope, but electrical sheets sometimes carry real HVAC-relevant facts:
- A circle is a common symbol for a ceiling-mounted fixture; a half-circle or similar with hatching commonly marks a wall-mounted fixture or switch; a rectangle with diagonal lines commonly marks an electrical panel. Circuit numbers and panel schedules are electrical design detail, out of scope.
- Worth scanning for: a labeled disconnect switch near an outdoor unit location (confirms where outdoor HVAC equipment actually sits), a dedicated circuit labeled with an HVAC equipment tag and amperage (confirms that equipment's real existence/location even if its own mechanical sheet is missing or unclear), or a thermostat wiring run. Extract only the HVAC-relevant fact these confirm (equipment location, existence) - never the electrical circuit/panel design itself.`;
}
