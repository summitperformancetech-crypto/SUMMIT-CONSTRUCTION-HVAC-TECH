...
export type ExtractedSheet = {
  ...
  // The sheet's own printed scale notation, copied verbatim from its
  // title block (e.g. "1/4\" = 1'-0\"", "3/16\"=1'-0\""), or null if this
  // sheet shows no scale callout (common for a schedule/detail sheet
  // with no plotted scale, or when it genuinely isn't visible). A plain
  // per-sheet transcription, same discipline as
  // ceiling_insulation_callout_text/revisionDate above - lib/ductRouting.ts's
  // parseArchitecturalScaleText/resolveSheetScale do the actual parsing
  // and prefer this over the AI-room-bounding-box median scale estimate
  // whenever it's present, since a printed scale is exact by
  // construction and the bounding-box method is a real, measurable
  // approximation (diagnosed 2026-08-25 against real Schneider data:
  // ~15% off even after the page-rotation fix).
  printed_scale_text: string | null;
};
...
Cross-sheet room lookup, before marking any room's dimension unresolved: read this document the way a licensed architect reviews a set, not the way someone skims a single sheet. A room's real dimension label is not always printed on the sheet where you first identify that room - a two-story room (e.g. a vaulted closet or a stairwell) can have its dimension label on an upper-level or bonus-room sheet instead of the main floor plan; a room can appear a second time, under a slightly different name, on a detail or partial-plan sheet with a clearer, dimensioned callout the main overview sheet lacks. Before setting a room's floor_area_sqft, wall lengths, or windows to unresolved, check every other sheet in this set for that same room (same name, a near-variant name, or the same physical location/footprint) and use whichever sheet actually carries a real printed dimension. Only mark it unresolved once you have checked the WHOLE document for that room, not just the one sheet you found it on first. When you do this, and when you consolidate what is genuinely the same physical room found under two different labels on two different sheets, only report it ONCE - do not emit both room[N] and room[M] output entries for what is provably a single physical space (state that reasoning in "reason" so a human reviewer can see why one label was preferred and the other suppressed).
...

{
  ...
  "sheets": [
    { "name": string, "sourceAuthority": "sealed_construction_document" | "reference_only" | "unknown", "ceiling_insulation_callout_text": string | null, "revisionDate": string | null, "revisionNote": string | null, "frontAnchorPageEdge": "top" | "bottom" | "left" | "right" | null, "page_number": number | null, "printed_scale_text": string | null }
  ],
  "building_envelope": {
