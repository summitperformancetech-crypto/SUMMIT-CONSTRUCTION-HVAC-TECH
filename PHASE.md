...
- **(Sixth entry, 2026-08-23)** Separately: should the review/resolution UI itself change so that "accepting" a room's bundled unresolved reason can't silently leave the specific structured fields that reason names (e.g. window area) still null? That's the deeper root cause; the completeness checker above only makes the symptom visible after the fact.
...
## Updated
Date/time: 2026-08-23 (sixth entry — root-caused the window-area gap definitively and shipped a tested, additive completeness-check module, `lib/dataCompleteness.ts`; see Deferred Work)
