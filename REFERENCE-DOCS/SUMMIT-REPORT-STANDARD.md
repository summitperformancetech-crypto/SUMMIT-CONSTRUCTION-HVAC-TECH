...
- Room-level data is actually complete, not just internally consistent (`checkDataCompleteness`, `lib/dataCompleteness.ts`, added 2026-08-23) — no conditioned room is missing floor area or wall geometry, and the whole house has nonzero total glazing area. Diagnosed against real production data: a project can pass every other check here, including the arithmetic check below, while several rooms have no floor area at all or the whole house has exactly zero window area despite real square footage — those are silent data gaps, not math errors, and need their own gate.
...
10. **Duct Routing** — added 2026-08-25 alongside the auto Manual D run-length feature. A real installation reference diagram: the actual source drawing page (never regenerated, same drawing-authority principle as §9) with the tech-confirmed AHU and room pins overlaid, and the routed (Manhattan) path between each labeled with its real length and diameter — the same figures shown in the Manual D schedule (§11). Pin positions are always human-confirmed or human-placed (see §7a) — never rendered from an unconfirmed AI suggestion. Renders an explicit "no pins resolved yet" state, never a fabricated or approximated routing, when a project hasn't used this feature.
11. **Duct Sizing Schedule** — added 2026-08-26 as part of the Permit-Submittable Manual D Package (§7b). A real table, one row per duct_runs segment: room, run type, material, CFM, size, length, fitting equivalent length, total effective length, friction rate, pressure drop, velocity. Every number traces to the same computeManualD/sizeDuctRun chain the diagram uses — no separate calculation.
12. **Register / Grille Schedule** — added 2026-08-26. Real duct_diffusers + duct_terminations rows: room, zone, direction, pattern type, size, CFM, hardware. A room with no diffuser entered yet does not appear — this schedule never falls back to the diagram's generic default symbol.
13. **Design Check Summary** — added 2026-08-26. Real pass/fail against every rule this app can actually verify (Equal Friction method applied, per-segment velocity limits, per-zone return-air balance with real numbers) — and explicit `BLOCKED`/`NOT APPLICABLE` sections, with the real reason stated, for checks the app cannot yet perform (ESP-vs-equipment-capacity gate: no equipment blower/ESP data sourced; extended-plenum/reducing-trunk/take-off-spacing/damper-placement rules: the routing engine's topology is a single home-run trunk per zone, not a modeled multi-segment trunk). See §7b.
14. **Extraction status / field reference** — per-level room confirmation count (confirmed / total), plus any field-verified overrides with reason.
15. **Audit Trail & QA Certification** — the automated cross-foot validation table (see §6) and a correction log for anything the engine adjusted, with before/after values and reason. This page is the core differentiator and must never be omitted.
...
## 7b. Permit-Submittable Manual D Package (added 2026-08-26)

Rebuild directive: Summit's Manual D output must be a permit-submittable design package, not just a diagram. Real, shipped this session, and real, honestly-disclosed gaps:

**Real and verified before this rebuild** (confirmed by direct code read, not assumed): Equal Friction sizing method (`lib/manualD.ts`'s `computeZoneFrictionRates`/`sizeDuctRun`), real ACCA Manual D Appendix 3 fitting equivalent-length data, the real ASHRAE round-duct friction correlation (cross-checked against 4 CED Engineering worked examples), real Huebscher rectangular-equivalent math, real ACCA-cited 1500/800 fpm trunk/branch velocity limits, and a real TESP/ASP formula.

**Shipped this session**: `duct_runs.total_effective_length_ft`/`pressure_drop_iwc` — the same length+EL and friction-rate×length math `sizeDuctRun` always computed, now persisted per segment as a real auditable trail (not a new calculation). `checkReturnAirBalance` (`lib/ductRouting.ts`) — real per-zone supply-vs-return CFM comparison from actual `duct_diffusers` rows, reporting "not yet determinable" rather than a fabricated pass when a zone has no real return-diffuser data. Three new report pages (§5 items 11–13): Duct Sizing Schedule, Register/Grille Schedule, Design Check Summary. `report_sign_offs` — a real licensed-reviewer sign-off gate (name, license number, license type, signed-by user, timestamp); export/submittable status must check for an active (non-superseded) sign-off against the current snapshot version, never represent an unsigned package as permit-ready.

**Confirmed, disclosed, NOT built** (verified by direct schema/code inspection, not fabricated around):
- **ESP-vs-equipment-capacity gate**: `equipment_catalog` and `equipment_performance_points` have no rated ESP or airflow-vs-static-pressure blower curve field for any unit. This gate cannot run until that OEM data is sourced per equipment line — the Design Check Summary shows this as `BLOCKED`, never a fabricated pass.
- **Extended Plenum / Reducing Trunk topology**: the routing engine's own code comment states the schema models "one trunk per zone feeding branches, not a multi-level trunk/sub-trunk hierarchy." Take-off spacing/offset/end-cap-clearance rules, reducing-trunk auto-switch at 24ft with 15–20ft step-downs, and per-branch balancing-damper placement all require a real multi-segment trunk topology this app does not yet model — a routing-model rebuild, not a report-page change. Shown as `NOT APPLICABLE` with the real reason, never checked against a topology that doesn't exist.
- **Independent 3-project validation** (real Manual D output checked against a licensed contractor's or Wrightsoft/Elite RHVAC/Cool Calc's independently-verified numbers) has not been performed — this requires real external comparison data the app cannot generate for itself.
...
- [ ] All 15 sections in §5 render with real project data, no placeholder or "TBD" text remaining anywhere in output.
