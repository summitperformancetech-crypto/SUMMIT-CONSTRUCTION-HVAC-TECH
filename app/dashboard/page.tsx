  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-semibold text-brand-gold">Projects</h1>
        <Link
          href="/dashboard/new"
      ...
      {!error && projects && projects.length === 0 && (
        <p className="rounded-md border border-brand-gold/50 bg-brand-bg px-4 py-6 text-center text-sm text-brand-grey-text">
          No projects yet. Create your first one to get started.
        </p>
      )}

      {!error && projects && projects.length > 0 && (
        <ul className="divide-y divide-brand-gold/50 rounded-lg border border-brand-gold/50 bg-brand-bg">
