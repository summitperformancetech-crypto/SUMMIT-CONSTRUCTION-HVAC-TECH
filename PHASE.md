...
**Git**: branch `main`, working tree has only doc updates + pre-existing untracked reference files pending (see below) once this handoff commit lands. **NOT pushed to `origin/main` this entire session** - `main` is 22+ commits ahead of `origin/main`. Never push without being asked first.

**Last 4 commits** (newest first): `c8d4b43` (remove room-name labels + anchor AHU trunk to real pin), `9cf1475` (crop diagram to each sheet's real floor-plan extent), `71a7ac8` (Manual D visual fidelity rework + 16 room-position corrections), `5afc504` (earlier: match diagram to IMG_3916.JPG standard).

**Untracked files intentionally left alone, not yet resolved**:
- `REFERENCE-DOCS/IMG_3933.HEIC`, `IMG_3934.HEIC`, `IMG_3935.HEIC` - the user's own before-screenshots from earlier this session, not referenced by any code, never committed (deliberate - only `IMG_3916.JPG`, the actual target standard cited in code comments, was committed).
- `manual D duct design diagram  /` (note: literal double space before the trailing `/` in the real directory name - use `ls` or Python `os.listdir`, not a typed path, to reference it) - a stray folder in the project root with 4 loose JPGs including a duplicate of IMG_3916. Never asked about; leave alone unless the user requests cleanup.

**Live app state**: dev server was restarted once this session (required to clear a stuck Turbopack compile, not a code issue) - if the user has the app open in a browser tab, it needs a refresh. Schneider project (`94749d9f-ecc1-4bb8-b38f-d927cb150b90`) is on `calculation_snapshots` version 4, generated fresh after the Round 3 fixes.

**The actual open problem**: the Manual D duct-routing diagram (both the live in-app view and the PDF report page) still doesn't satisfy the user visually, and as of this session's last message they had not yet said what specifically is still wrong. Three prior rounds of "this is wrong" each turned out to be a specific, real, fixable bug (not a subjective taste issue) - so the right posture for the next session is to keep treating new complaints as pointing at real, findable bugs until proven otherwise, not to assume it's unfixable or purely aesthetic.
