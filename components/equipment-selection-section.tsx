...
const TYPE_LABEL: Record<EquipmentCatalogEntry["equipmentType"], string> = {
  ...
  coil: "Coil",
  // Never actually rendered here - makeup_air_unit/exhaust_fan/
  // dehumidifier rows are excluded from this component's ranked/air-
  // handler lists above (see lib/makeupAir.ts, components/makeup-air-
  // section.tsx, and components/dehumidification-section.tsx instead).
  // Present only because TYPE_LABEL is a Record over the full
  // EquipmentType union.
...
export function EquipmentSelectionSection({
  ...
  const ranked = useMemo(() => {
    ...
    // exhaust_fan, and dehumidifier rows are real catalog entry types too
    // (see lib/makeupAir.ts, components/makeup-air-section.tsx, and
    // components/dehumidification-section.tsx) but are never a Manual S
    ...
    const evals: EquipmentEvaluation[] = catalog
      .filter(
        (equipment) =>
          ...
          equipment.equipmentType !== "exhaust_fan" &&
          equipment.equipmentType !== "dehumidifier",
      )
      .map((equipment) =>
      evaluateEquipment(
        equipment,
