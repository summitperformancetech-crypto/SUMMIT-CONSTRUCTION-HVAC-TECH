...
## 2026-08-25/26 — Manual D visual fidelity rework (return plenum, exact scale, pin canvas) + 16 room-position corrections

User rejected the duct-routing diagram's visual quality outright ("this still looks awful"), then gave a specific real target standard already in the repo (`REFERENCE-DOCS/IMG_3916.JPG`, a genuine Wrightsoft/AutoCAD-style schematic) and current-state screenshots from their own device, with an explicit instruction to report a full gap list and rendering plan before touching code. Gap list covered: register symbols (red square+X with throw tick vs. a plain circle), return-air representation, flex vs. metal duct line texture, stacked register callouts (circled type code + size/CFM), uppercase+underlined room labels, and page scale/resolution. User then authorized implementation and asked to see the finished result.

A follow-up, highly detailed prompt raised six concrete requirements, one of which (a claimed 612×792pt/Letter page size) was independently fact-checked against the real PDF and found wrong — the real Schneider sheet is 2592×1728pt raw (36"×24" D-size, `/Rotate 270`), reported to the user with evidence before building anything on the incorrect premise. Independently found a much better real fact the user's prompt hadn't mentioned: the sheet's actual printed scale notation, "1/4\"=1'-0\"", visible in both title blocks — exact 24px/ft at 96dpi, vs. the prior AI-bounding-box-median estimate's ~15% error.

**Shipped, verified:**
1. **`parseArchitecturalScaleText`/`pageScaleFromArchitecturalScale`/`resolveSheetScale`** (`lib/ductRouting.ts`) — real printed-scale-notation parsing wired into the live routing/report flow, preferring it over the old bounding-box-median estimate whenever a sheet's `printed_scale_text` is present. `lib/drawingExtraction.ts`'s extraction schema/prompt extended to capture this field going forward.
2. **Return-air plenum as a first-class, required-per-zone pin** — new `zones.return_position_x_norm/y_norm/source_drawing_id/source_page_number` columns (migration `20260826000000_add_zone_return_position.sql`), full pin-canvas workflow identical to the existing AHU pin (place/drag/save with a required reason, `field_resolutions` audit row, `field_name: "return_position"`), `getDuctRoutingGateStatus` now requires it resolved too. Placed through the real live UI via an authenticated Puppeteer session (not a direct DB write) since no mechanical/return-air schedule exists in this drawing set to source a position from more precisely — same standing principle as every other resolved field in this app.
3. **Pin canvas redesign** (`components/duct-routing-canvas.tsx`) — replaced pill-shaped buttons with small SVG crosshair+circle markers (return-kind renders as a square), only the active pin shows a floating label.
4. **Rendering fidelity, matched in both `components/duct-routing-diagram.tsx` and `lib/reportHtmlV2.ts`** — register symbols redrawn (red square+X+throw tick), new return-air grille symbol (green outlined square, diagonal line, "RA") at the zone's own real return pin instead of assumed co-located with the AHU, dashed flex-duct texture on branch/run-out vs. solid metal trunk, stacked register callouts (circled type code + size + CFM), uppercase+underlined room labels (required a `CHAR_WIDTH_FACTOR` bump from 0.58→0.68 after a live screenshot caught overlapping labels post-uppercasing), updated legend.
5. **Room pin position corrections (16 of 26 rooms)** — while running the user's own explicit acceptance test (open a sheet, confirm named rooms' pins sit inside their real walls, report exact pixel coordinates for independent spot-check), found the underlying AI-extracted `rooms.position_x_norm/y_norm` values were widely wrong across both sheets — outside the building, on a dividing wall, or inside the adjacent room — a real, pre-existing data-quality issue unrelated to this session's other work, not a single systematic transform bug (per-room, independent misreads). Per the project's standing drawing-reading requirement, re-read every room directly against the full-resolution rendered sheet at the now-exact scale, computed corrected pixel coordinates, and cross-checked each visually (crosshair overlays against the drawing's own thick wall lines) before writing anything. Two corrections (Bedroom 3, Stairs 2nd Floor) were caught as still-wrong by the user on a first visual review pass and re-derived at 2x zoom directly against the wall lines - not the dimension lines, which had been the misread the first time. All 16 written via the same disclosed `field_resolutions` mechanism (`resolution_type: "overridden"`, real previous-value + reason).
6. **Visibility discipline learned mid-session**: images loaded via the Read tool are tool output only, not visible to the user in chat — the first two "here's a screenshot" attempts genuinely showed nothing on the user's side. Fixed by publishing an actual Artifact (embedded images, before/after crosshair comparisons) for every subsequent visual check, including the final live-generated PDF pages pulled from a real `/api/reports`/`/api/reports/revise` call (new calculation snapshot v2, real revision reason logged) rather than a mockup.

**Verification**: `tsc --noEmit` clean, full suite 250/250 (0 regressions), production build clean. Live-verified via real authenticated Puppeteer sessions throughout (not direct DB writes for anything a technician would normally enter through the UI), plus a real end-to-end PDF regeneration confirming the finished duct routing pages render correctly with every fix applied.

Next action: commit this workstream (return plenum + scale fix + pin canvas redesign + room-position corrections) as a real checkpoint.

**Committed as `71a7ac8`.** User then reviewed the live output and found it genuinely still wrong three more times in a row, each a real, distinct bug - not the same complaint repeated:

**Round 2 - Sheet 2 "out of scale."** Diagnosed: the underlying feet-per-pixel scale was identical and correct on both sheets (verified independently), but Sheet 2's real floor plan (688 SF) occupies a small corner of the same full 36"x24" sheet A3.0's 2,350 SF plan fills almost entirely - so the same fixed-size schematic register/label symbols (correct on Sheet 1) looked oversized next to Sheet 2's much smaller drawing. Fix: new `computeSheetCropViewBox` (`lib/ductRouting.ts`) crops/zooms each sheet's own SVG viewBox to a square region around its real pin extents, with every fixed-size symbol constant shrunk by the resulting zoom factor so it renders at its original absolute size - applied identically to `lib/reportHtmlV2.ts` and `components/duct-routing-diagram.tsx`, with the underlying image given a matching CSS `transform: scale() translate()` so the raster floor plan and vector overlay stay in lockstep. Committed `9cf1475`, verified via a fresh snapshot (v3) and a real regenerated PDF.

**Round 3 - two more real bugs, caught together.** (a) Room-name text labels ("MASTER BEDROOM", "BEDROOM 5", etc.) were floating over the diagram - directly contradicting an explicit instruction from earlier in this same session ("draw ONLY supply/branch lines, registers, callouts, AHU, and the return-air plenum symbol - rely on the PDF's own printed room labels, don't invent a second set") that got lost during the Round 1 rebuild. Fixed by deleting the room-label-candidate code in `layoutDuctRoutingLabels` entirely (register/trunk size-CFM callouts untouched). (b) The user's own theory - "the file that was uploaded that determined the coordinates" was the real issue - was correct and led straight to the actual bug: the real, human-digitized `/Users/JohnHarper/Downloads/duct routing graph.json` (confirmed via session transcript grep to be a genuine user-supplied file, faithfully transcribed into `zones.corridor_graph` - not a fabrication) explicitly documents itself as "a solid scaffold (±1ft), not survey-grade," and its own AHU coordinate carries an explicit disclaimer per zone ("placed... per Summit's existing AHU pin" / "NEEDS PLACEMENT in Summit per your pin list... suggested near stair landing, verify attic access clearance") - i.e. the file's own author intended the app's real confirmed AHU pin to take precedence. The code wasn't doing that: `computeSegmentsFromCorridorGraph` used the graph's own calibrated AHU coordinate for the trunk's start point regardless. Measured and visually confirmed against the real drawing: this put the trunk's start ~19ft from the real confirmed AHU pin on Zone 1 (landing inside the Kitchen instead of the real Utility Room) and inside the stairwell instead of the real Unfinished Attic on Zone 2 - a visible disconnect between the AHU icon (drawn from the real pin) and where the duct trunk visually began. Fixed by threading the zone's real `ahuPoint` through as an override (`lib/ductCorridorGraph.ts`'s `buildSegmentsFromCorridorGraph`/`computeSegmentsFromCorridorGraph`, `lib/ductRouting.ts`'s one call site) - falls back to the graph's own calibrated point only when no real confirmed pin exists. Committed `c8d4b43`.

**A real, disclosed infrastructure hiccup during Round 3 verification**: regenerating the report via the live dev server returned a raw 500 with no JSON body (confirmed via a standalone `vitest` run of the exact same `getReportData`/`attachFrozenImages` pipeline against real Schneider data succeeding cleanly - the code itself was correct). Diagnosed as the long-running dev server (up since the start of this session) stuck mid-recompile under this machine's already-documented severe disk I/O contention, not a real regression. Restarted the dev server (`kill` + `nohup npm run dev`), waited ~90s+ for Turbopack's own "Compiling proxy..." step to clear under load, then successfully regenerated. **User-facing consequence, disclosed to the user directly**: this restart means anyone with the live app open in a browser tab needed to refresh - flagged, not silently done.

**Verification after each of the two Round-2/3 commits**: `tsc --noEmit` clean, full suite clean each time (255/255, then 258/258 - 0 regressions), production build clean, and - critically, per this project's own "never claim fixed without live proof" standard - a fresh `calculation_snapshots` version regenerated via the real `/api/reports/revise` + `/api/reports` endpoints each time (v3, then v4), with the actual resulting PDF pages pulled out and visually re-inspected before reporting anything fixed to the user.

**Status at end of session: still not resolved to the user's satisfaction.** After Round 3's fixes were shown, the user said: "ok, i am still not satisfied with the look of the diagram" - no further specifics given before asking for a handoff to a new chat. This is NOT the same failure mode as Rounds 1-3 (each of which was a specific, locatable, provable bug the user could point to or that showed up unambiguously in a side-by-side comparison) - at this point the remaining dissatisfaction is either (a) a specific issue not yet articulated, or (b) a more fundamental aesthetic/rendering-approach gap that incremental bug fixes on the current architecture won't close. **The very next thing the new session must do is get concrete, itemized feedback from the user on what specifically still looks wrong** (matching the original Round-1 pattern that worked well: ask for a real gap list against the reference standard, or a fresh marked-up screenshot) - do not guess and start changing code without that.

## 2026-08-26 — Permit-Submittable Manual D Package: full rebuild (10 commits)

New session, continuing directly from the handoff above (same day). Rather than giving the requested itemized diagram feedback, the user escalated to a full rebuild spec covering the entire Manual D pipeline, not just the diagram: **"Rebuild Summit's Manual D engine and output pipeline to produce a permit-submittable HVAC design package"**, 8 sections — (1) master ACCA Manual D first, (2) source real calc tables, never fabricate, flag missing data explicitly, (3) full auditable per-segment calc chain, (4) real Extended-Plenum/Reducing-Trunk system-design rules (24ft/48ft single/double-run limits, 15-20ft reduction steps, trunk dimension/take-off-spacing rules, balancing dampers, return-air CFM balance), (5) ESP-vs-equipment-rated-capacity hard gate with a 10-15% safety factor, (6) full permit package output (J summary, S sheet, D schedule table, routing diagram, register schedule, design check summary), (7) a licensed-reviewer sign-off gate before permit-ready export, (8) validation against 3 real completed projects with independently-verified Manual D numbers.

When gaps were initially flagged as blockers (equipment ESP/blower data, trunk-topology model, external validation data), the user pushed back sharply: **"If there is information that you need in order to do your job, then source it... I'm not asking you to fabricate anything that's not accurate. So tell me what exactly you need from a licensed contractor."** — the standing instruction for the rest of this session: source real data actively rather than passively citing blockers, and precisely name what's genuinely still missing rather than re-requesting what's already available. Later, once Vivian Street's real source PDFs were found already in the repo and confirmed to contain zero Manual D content, the user said **"THEN DONT WORRY ABOUT THE MANUAL D FOR VIVIAN PROJECT"** — dropping it from Section 8's validation scope (Manual J only). The user also required real-time visual progress tracking ("I WANT TO SEE VISIBLY SEE WHAT THE PROGRESS LOOKS LIKE AS IT HAPPENS") — a "Schneider Manual D Set" Artifact styled as a construction-permit sheet index was published for this.

**Shipped, verified, committed (10 commits, oldest first):**

1. **`1fe5756` — real diffuser/hardware/termination schema.** New reference tables (`duct_diffuser_pattern_types`, `duct_material_specs`) seeded with real cited data (UL 181 flex-duct R-values, NAIMA AH119/AH136 duct-board R-value/inch), plus `duct_diffusers`, `duct_terminations`, `ahu_installation_detail`, and org-level defaults tables.
2. **`e458ec8` — real per-diffuser pattern types** replace the old one-way-only diagram default, wired into both renderers.
3. **`55b3ed3` — AHU installation detail entry UI** and an auto-generated report section from it.
4. **`a262b2e` — derive duct-routing-mode basis from real data** (Extended Plenum vs. Reducing Trunk) instead of asking the user a new redundant question.
5. **`cc9c40f` — real duct terminations entry UI and plan rendering** (exhaust fan/dryer vent/ODA intake/condensate discharge symbols).
6. **`2e6f473` — the permit-package spine**: auditable per-segment trail (`duct_runs.total_effective_length_ft`/`pressure_drop_iwc`), 3 new report pages (Duct Sizing Schedule, Register Schedule, Design Check Summary), and the `report_sign_offs` table + banner (DRAFT vs. REVIEWED-AND-APPROVED on every page) — the licensed-reviewer gate from Section 7.
7. **`b0cac64` — real ESP-vs-equipment-capacity gate** (Section 5): `equipment_catalog.equipment_type = 'air_handler'` added as a new, non-capacity-bearing line (excluded from equipment-ranking pools at both call sites); real Goodman AVPTC blower-performance data (108 points, 3 models) transcribed from Goodman's own SS-GAVPTC spec sheet; `checkEspVsEquipmentCapacity` with a disclosed 12.5% safety factor (midpoint of the user's stated 10-15% range).
8. **`72d6d28` — real Extended Plenum/Reducing Trunk topology analysis** (the rest of Section 4): `lib/ductTrunkTopology.ts` built directly on the existing real, human-digitized `corridor_graph` (never the approximate computed room-avoidance router, which has no notion of "where along the trunk" a branch attaches) — real per-arm length/classification, evenly-spaced reduction points, and take-off spacing-violation checks against real plenum-end/terminal-end/post-reduction clearances. 19 new tests against Schneider's real Zone 1 graph (hand-traced arm lengths); one of the author's own hand-trace assertions was caught wrong by a failing test and fixed.
9. **`26e8ea5` — real reducer/violation markers drawn on the diagram** (the visual completion of Section 4): tapered-reducer glyphs labeled with real downstream CFM, and dashed-red-ring violation markers, in both the PDF and live-app renderers. **A real, disclosed-mid-implementation bug was caught during live verification against Schneider and fixed in the same commit**: the corridor_graph's own room ids are human-readable slugs in the digitizer's space (e.g. `"master_bedroom"`), not this app's real room UUIDs — every downstream lookup keyed by that id (CFM sums, the violation-flag comparison against the diagram's real route data, and the Design Check Summary table's diameter-based clearance check) was silently missing every match. Before the fix: every reducer showed `downstreamCfm: 0`, and the "violation ring" count in the rendered HTML was really just the legend's own sample icon, not a real marker. Fixed with a new name-matched bridge (`mapGraphRoomIdsToRealRoomIds`, the same matching approach `fitCorridorGraphCalibration` already used for its own anchor points) threaded through all three call sites. After the fix: real non-zero downstream CFM (198.9/272.2/85.2 across the two real sheets) and 11 real violation rings, verified via a temporary scratch script against the live Schneider DB (deleted after use).
10. **`0fa473d` — real blower data for the other 4 requested manufacturers** (Amana, Carrier, Daikin, Trane) plus 2 more Goodman models. Real, disclosed finding: current-generation Amana (SS-AAVPTC) and Carrier's newer communicating fan coil (FE4A/FE5A, checked and rejected) don't publish an open ESP-swept curve at all anymore — their ECM blowers self-regulate to a constant target CFM within a documented recommended range, so Amana was modeled honestly as two points holding that real constant value at the range's endpoints, and Carrier's older multi-tap FB4C line was sourced instead (it does publish a real 5-tap swept table). Daikin's own separately-published SS-DVPTC (same Daikin-owned platform family as Goodman/Amana, verified independently rather than copied) does still publish a full 4-tap curve. Trane's TAM7 uses field-configurable CFM/ton settings rather than lettered taps — modeled as 8 real settings, using only the conservative "Constant CFM Mode" value where the source table publishes a paired mode value. All 10 models across 5 manufacturers applied directly to the live DB and verified via a live query (expected point counts confirmed) before committing.

**Verification held throughout, every commit**: `tsc --noEmit` clean (after the documented `rm -rf .next && npx next typegen` step each time), full suite clean and growing with zero regressions (324/324 at session's end, up from 258 at the last checkpoint), production build clean. Live-verified against the real Schneider project via temporary scratch scripts (deleted after use, no permanent DB writes) at multiple points, not just static checks — this is what caught the room-id bridging bug in commit `26e8ea5` before it reached the user.

**Not yet done — genuinely open, not silently dropped:**
- Section 8's 3-project independent-validation requirement still has no real project with an independently-computed Manual D duct schedule to check against (Vivian Street explicitly excluded per the user's own instruction; still Manual-J-only).
- The "Schneider Manual D Set" Artifact was published early in this session but not redeployed to reflect the later ESP-gate/trunk-topology/reducer/equipment-sourcing commits — the user's own "see progress as it happens" requirement is only partially honored as of this checkpoint.
- The pre-existing "diagram still doesn't look right" complaint from the prior session's handoff was not directly re-litigated — the user's rebuild-spec instruction superseded it, but no explicit confirmation was sought that the rebuilt diagram (with return-air balance, reducers, violations) now satisfies the original visual complaint.
- Register/grille schedule report page (Section 6) and the full permit package assembly were built as part of `2e6f473`, but a true end-to-end "generate one document, hand it to a reviewer, get a real sign-off, export as permit-ready" walkthrough has not been performed live.

Next action: report this checkpoint to the user (10 commits, real sourcing across 5 equipment manufacturers, real trunk-topology/ESP gates, a real bug caught and fixed via live verification), then get direction on which open item to take next — the Artifact redeploy, the 3-project validation sourcing, or resuming the diagram's own visual-quality thread now that the underlying engine is substantially more complete.

## 2026-08-27 — Catalog Expansion + Recommended Install Package (7 commits)

New session (same day). User asked for a visual illustration of Summit's equipment catalog plus what's missing to install correctly per manufacturer spec — built and published an artifact (18 rows, 5 manufacturers, real gap list). User then handed over a full build spec (`SUMMIT-CATALOG-AND-INSTALL-PACKAGE-BUILD-SPEC.md`, committed to the repo) closing those 9 gaps, expanding catalog depth, and building a **Recommended Install Package generator** — turning a Manual S compatibility match into a real, purchasable bill of materials. The spec's own protocol required a diagnostic report confirming current schema state for each gap before any code changes; delivered that first (found one imprecision worth correcting: `equipment_type` already serves as the "category" dimension the spec assumed didn't exist, and a `coil` type — not a new enum — was the real missing piece; also flagged a load-bearing prerequisite the spec's own gap list missed: no condenser/outdoor-unit position pin exists anywhere, so line-set length can never be computed without one). User's answer: **"continue to completion. do what you need source whatever info you need uninterupted to achieve this."**

**Shipped and committed, in order:**

1. **`7817e36` — schema for 6 gap categories.** `equipment_electrical_specs`, `refrigerant_lineset_specs` (made nullable on `max_equivalent_length_ft` after real sourcing immediately hit a manufacturer that doesn't publish it — `20260827105000` fix), `equipment_coil_matching`, `equipment_heat_kit_compatibility`, `equipment_filter_specs`, `diffuser_hardware_catalog`/`duct_material_hardware_catalog` (real shared SKU catalogs, with an additive `catalog_id` FK on the existing org-default tables), a new `coil` `equipment_type` (excluded from Manual S ranking exactly like `air_handler`), and `zones.condenser_position_*` (the flagged prerequisite, mirroring the existing AHU/return-plenum pin columns).
2. **`3739d17` — real Daikin and Trane outdoor units** (the highest-priority gap per the spec). Daikin: 3 real DZ4SE single-stage heat pumps (2/3/4-ton) plus their real matched cased coils as new `coil` rows, each with full 41-point real performance curves transcribed from `SS-DZ4SE.pdf`, real electrical/lineset data, and real AHRI-implied coil-matching sourced directly from each Cooling Data table's own header (not inferred from tonnage). Trane: 2 real XR15 (4TWR5) heat pumps with real electrical/lineset — checked Trane's residential literature across two XR15 revisions plus the prior XR14 generation and confirmed none publish an open extended-performance table (Trane defers that to dealer-only TRACE software); zero fabricated performance points, verified safe (equipment with no points simply never ranks, doesn't crash).
3. **`39136fb` — electrical/lineset/filter/heat-kit backfill for the 18 pre-existing rows.** Re-fetched each row's own already-cited source document rather than estimating. Every one of 12 lineset rows has a null `max_equivalent_length_ft` — a real, consistent finding across all 5 manufacturers' consumer literature (only Trane's XR15 doc gave an actual max length/lift figure), not a one-off gap.
4. **`16a8c1b` — real diffuser/duct-material hardware catalog** (Gap 7). 7 real products (Hart & Cooley 301/682M/303/304M/631 Series, Titus ML-39, Titus 350RL) and 4 real duct-material products (Thermaflex M-KE, Imperial Manufacturing Group Snap Lock Round Pipe, Johns Manville Linacoustic R-300, CertainTeed ToughGard R), sourced across the exact national suppliers the user named. Metal-Fab's real product line (gas venting) didn't fit either catalog and wasn't forced in — flagged as needing a vent/flue hardware catalog this session didn't build, since furnaces (which would need it) aren't catalogued yet either.
5. **`357d4b3` — the Recommended Install Package generator itself**, the spec's core new capability. `lib/installPackage.ts`'s `computeInstallPackage` implements the real 8-step assembly (coil-matching hard-flag, electrical, lineset sizing against a real run length, heat-kit selection against the real Manual J/S deficit and the kit's own minimum-airflow requirement, filter, diffuser/duct-material/termination inheritance from already-resolved Manual D data, and a completeness score that treats a flagged item as incomplete same as unresolved — matching the spec's own worked example). 12 new direct unit tests. Wired into `getReportData` (reusing the already-computed Manual S `supplementalHeatBtuh` rather than re-deriving it) and a new report page. Live-verified against the real Schneider project: 2 zones, correct real output reflecting Schneider's actual state (16.7% complete on both — 1 of 6 line items resolved, matching the real math).

**A real environmental hiccup, resolved before this session ended**: partway through building the install-package commit, this machine hit a severe, sustained slowdown — `tsc --noEmit` took over 20 minutes with almost no CPU progress across two separate attempts (retrying fresh didn't help), the full `vitest` suite took 10-70x its normal ~2s runtime, and a `next build` attempt was still mid-compile after 15+ minutes. Diagnosed as genuine environmental/system resource contention, not a code issue: a plain `uptime` command also stalled at one point, and even a single isolated test file that normally runs in ~300ms took 26-73 seconds. The new module's own 12 tests DID pass, and the live scratch-script verification against real Schneider data DID succeed with correct output throughout, so the `357d4b3` commit landed on genuinely correct code - but without a confirmed clean build, disclosed explicitly rather than assumed. Once the environment cleared, the stalled `next build` finally completed and correctly caught one real, expected issue: `reportGate.test.mts`/`reportValidation.test.mts` needed the new `installPackagesByZone` field added to their hand-built residential fixtures - the same fixture-drift pattern this session hit repeatedly elsewhere. Fixed in `aa8edde`. **Final state, genuinely confirmed**: `next build` compiled in 1.7s with its TypeScript step finishing in ~3s, full suite 336/336 passing (23 test files, up 12 from the pre-install-package 324 - exactly the new module's own tests).

**Genuinely open, not silently dropped:**
- Catalog depth: only 3 Daikin + 2 Trane models added (not the spec's "top 7 per manufacturer per category"); Amana/Carrier/Goodman weren't expanded further this pass (already had reasonable depth from prior sessions).
- The condenser/outdoor-unit position pin has schema but no placement UI (no pin-canvas workflow like the existing AHU/return-plenum pins) - every project's refrigerant-lineset line item will read "run length not yet known" until that's built.
- `diffuser_org_defaults`/`duct_material_org_defaults` still have 0 rows on every org (confirmed live both before and after this session's hardware-catalog seeding) - the install package's duct-material line item will always read "unresolved" until an org actually sets a default.
- Section 6's UI (catalog browse improvements, an interactive package-review/swap-and-rescore screen, the org equipment-defaults settings extension) was not built - the generator and its schema are real and complete; the UI around them is scoped-out follow-up.
- The uncertified-pairing acknowledgement workflow has real schema (`install_packages.uncertified_pairing_acknowledged_*`) but no UI action to actually record one yet.

Next action: report this full, now-verification-complete checkpoint to the user and get direction on which open item to prioritize next (condenser pin UI, org hardware defaults, further catalog depth, or the install-package review UI).

## 2026-08-27/28 — Standing Equipment-Documentation Sourcing Sweep: Furnace Combustion-Safety Data (7 platforms) + GitHub Push

New standing workstream, distinct from the Phase 7 gap list and its addenda above — triggered by a direct, all-caps user directive after a real dehumidifier max-design-ESP miss was caught (a spec sheet omitted a real static-pressure ceiling that only the separate installation instructions published): **"SCRAPE everything about every model of product ever put into summit in the past, present, and future. YOU DO NOT SHORTCUT. YOU SOURCE EVERY BIT OF DETAILED INFORMATION..."** — establishing full-documentation sourcing (spec sheet + installation instructions + engineering/submittal data + service/parts manual, not just whichever loads first) as permanent, ongoing practice, not a one-time backfill. Persisted as memory `standing_full_documentation_sourcing_requirement.md`. A follow-up instruction set the sequencing: **"PRIORITIZE BASED ON THE SAFETY-RELEVANT GAPS FIRST THEN FINISH WORKING THROUGH THE REST."**

**Order worked, safety-first as instructed:**
1. Exhaust fans (bathroom/kitchen) — closed out. Corrected HVI-certification and/or backdraft-damper presence for Broan 505, Broan-NuTone BE6/BE8, Panasonic FV-0511VF1/FV-0511VK2 from each unit's own real installation instructions/spec sheet (not just a product page). Two genuine non-findings disclosed rather than guessed: Zephyr CBI-600A (bare blower component, HVI/damper concepts may not cleanly apply) and Broan RHVB600SSV (no model-specific manual locatable after multiple searches).
2. Makeup-air units — found two real, previously-cited-but-never-persisted gaps: a prior session's own migration comment had referenced a real BPI depressurization-limit table and IECC climate-zone makeup-air-flow-cap table in Broan's Application Guide that were never actually stored — added as new reference tables (`makeup_air_depressurization_limits`, `makeup_air_climate_zone_flow_caps`), disclosed as not yet wired into a per-project check. Fantech MUAS 750/1200: sourced real max CFM, duct diameter, and operating-temperature range from the real IOM (not on the product page); found a real, distinct 55°F minimum entering-air-temperature figure (vs. ~60°F cited elsewhere for other brands) and Fantech's own stated position against using central HVAC ductwork for makeup-air conveyance — both disclosed as genuine manufacturer differences, not reconciled into one number.
3. **Furnace gas-combustion-safety schema added.** Real facts specific to gas-fired equipment — ANSI Z21.47 venting category (I–IV), gas manifold pressure by fuel/stage, gas supply (inlet) pressure range by fuel (natural gas and propane have materially different real ranges, never assumed proportional), and a structured per-orientation clearances-to-combustibles object — had no columns to live in at all before this. New table `equipment_combustion_specs` (migration `20260827400000`). Caught and fixed a real design mistake before populating data: the schema originally had one generic gas-supply-pressure pair; the first real source document showed natural gas and propane have materially different published ranges, so it was corrected live to fuel-specific columns before any data was inserted.
4. **Furnace platforms sourced, one real Installation Instructions manual read per platform, never assumed identical across shared-OEM-platform siblings without an independent citation** (7 platforms total, all Amana/Daikin/Goodman — the entire identifiable footprint of that family in this catalog):
   - *MVM97/*CVM97 M-series modulating 97% AFUE, Category IV (`20260827410000`, IOG-2007).
   - *R9S80/*D9S80 single-stage 80% AFUE, Category I (`20260827420000`, IOG-2035B) — also independently confirmed the Goodman GR9S800803B row a prior session had flagged as inferred-not-verified.
   - *R9S96/*D9S96/*R9S92 single-stage 96% AFUE, Category IV (`20260828010000`, IOG-2037B).
   - *R9T96/*D9T96 two-stage 96% AFUE, Category IV (`20260828020000`, IOG-2038B) — real, notable finding: despite a genuine two-stage gas valve, the manual's own startup procedure targets the same manifold pressure for both low and high fire (staging achieved by burner/orifice count, not a lower low-stage pressure) — stored as identical low/high values per the document, not omitted.
   - *RVM97/*DVM97 R-series modulating 97% AFUE, Category IV (`20260828030000`, IOG-2045) — a real, distinct sibling of the M-series platform above, with its own different clearance table (no "Upflow Horizontal Alcove" position) and a wider published manifold-pressure range spanning real 50%/100% modulation points.
   - *DVT96/*RVT96 variable-speed two-stage 96% AFUE, Category IV (`20260828040000`, IOG-2044) — real, notable finding: genuinely different low-stage vs. high-stage manifold-pressure targets (1.9"/3.5" wc natural, 6.0"/10.0" wc propane), unlike the non-variable-speed *R9T96 platform above.
   - *R9S96*U California-only Ultra Low NOx single-stage 96% AFUE, Category IV (`20260828050000`, IOG-2040C) — real, notable finding: natural-gas-only with no propane variant at all (confirmed by the complete absence of a propane row in the unit's own gas-supply-pressure table — propane fields correctly left NULL as a real "does not exist" fact, not a gap), and a genuinely lower/narrower manifold-pressure target than the standard *R9S96 platform due to a distinct premix burner.

**A genuine, disclosed gap left unresolved rather than guessed**: Daikin's `DR96TC0803BN` and `DR97MC0803BN` catalog rows don't match either sourced manual's brand-wildcard naming convention (`*R9S96`/`*R9T96`/`*RVM97`, which implies Daikin's own equivalent would be `DR9S96...`) — left unassigned to any platform pending a real Daikin-specific document, not guessed onto either the single- or two-stage tier.

**Started but not completed**: Carrier's 58SB0/58SB1 platform. Found the real Installation, Start-Up, Operating and Service and Maintenance Instructions (Catalog No. 58SB0A-04SI) — confirmed Category I, and a real natural-gas manifold-pressure range (3.2-3.8" wc) matching the Goodman-family platforms — but hit two genuine, disclosed blockers: (1) the catalog's own model number (`58SB0090M17-14`) uses motor code "M", while this specific manual is explicitly the "A" revision with an "E" (ECM) motor code — an unconfirmed match, not assumed identical; (2) the real clearances-to-combustibles table (Fig. 1) is a graphical diagram whose numeric values didn't extract as readable text this session — left as a genuine gap rather than guessed from the Amana/Goodman family's own numbers (a different manufacturer's cabinet). Remaining furnace backlog: the rest of the Carrier lineup (~10 more platforms) and the full Trane lineup (3 platforms) — none started.

**Verification**: `tsc --noEmit` genuinely clean (confirmed via a direct, unpiped exit-code capture after an earlier piped `| tail` run masked a real signal — see Known Issues). `vitest run`: all 127 real tests passed across all 15 test files; the run's own non-zero process exit code came from 3 "worker failed to start / timeout" infrastructure errors under the same severe system load documented in prior sessions (one run took 257s, almost all in module-import time, not test execution) — confirmed unrelated to this session's changes, which are pure additive SQL migrations touching no application code. Every migration applied live via the standard `pg`-based Node script and verified via a direct REST query against the live table before being committed.

**Git**: 6 commits this session (`fabd70d` schema + first M-series batch, `ef6bcfd` R9S80, `c05de4c` the 5-platform batch above — plus 3 earlier exhaust-fan/makeup-air commits `88a5647`/`e850ffb`/`b747fcd`/`3cd7e90` from the same standing-sourcing directive). **User then explicitly instructed pushing to `origin/main` going forward, superseding the project's prior "never push without being asked" default for this and all future work**: "push everything to GITHUB that we have done as well as anything we do moving forward." All 70 outstanding local commits (this sweep plus everything from Phase 7 addenda that had never been pushed) were pushed in this session — `git push origin main` timed out twice under the same system load before a longer-timeout background run succeeded; `origin/main` is now confirmed in sync with local `main` at `c05de4c`.

**A real, disclosed process-tooling finding worth carrying forward**: a background `npx tsc --noEmit 2>&1 | tail -40` run printed real `TS6053: File not found` errors for 10 core lib files (`manualJ.ts`, `manualD.ts`, `manualS.ts`, `manualN.ts`, `manualNSimulation.ts`, `manualIndustrial.ts`, `localExhaust.ts`, `makeupAir.ts`, `offlineQueue.ts`, `offlineSync.ts`) alongside `[exited with code 0]` — investigated immediately rather than dismissed; confirmed via `ls -la lib/` that every file genuinely exists with a real, recent size/mtime, and that the "exit code 0" was `tail`'s own exit code, not `tsc`'s (a classic pipe-exit-code-masking bug) — the file-not-found errors were themselves a transient filesystem glitch under the same heavy system load (`git status` timed out at 2 minutes around the same time), not a real regression. Re-ran `tsc` with output redirected to a file and the exit code captured directly immediately afterward, confirming a genuinely clean 0-error result. **Lesson for future sessions**: never trust an exit code read after a `| tail` (or any pipe) — capture `$?` immediately after the command that actually matters, or redirect to a file instead of piping.

Next action: report this checkpoint, then continue the furnace-platform sweep (Carrier motor-code question first, then the rest of Carrier, then Trane), continuing to push after every commit per the new standing instruction.

## 2026-08-30 — Standing Documentation Sweep: Carrier 58SB0/58SB1 Furnace Combustion Safety (motor-code blocker resolved)

Continuation of the standing full-documentation sourcing sweep. User's call this session: **resolve the Carrier 58SB0 blocker** (motor-code question + graphical clearances table), then continue the Carrier/Trane furnace combustion-safety pass.

**Blocker resolved — motor code "M".** The prior session could only find the *superseded* **58SB0A** revision's Installation Instructions (`58SB0A-04SI`), whose model-number nomenclature (drawing A190041) defines the motor-type position as **C / E / V only** (E = Fixed-Speeds Constant Torque ECM) — no "M" code — so the match to the catalog's `58SB0090M17-14` was correctly left unconfirmed rather than guessed. This session pulled the **current 58SB0B/58SB1B Product Data (A190411, drawing A220581)** — the exact document already cited on those two `equipment_catalog` rows — which defines the motor-type position as **C = VCA ECM / V = VCT PWM / M = Multi 18-Speed Constant Torque (MCT) ECM**. So "M" *is* a real, current Carrier code; the 58SB0B revision re-labelled that position relative to 58SB0A. `58SB0090M17-14` is a 58SB0B-platform furnace (Comfort 80, single-stage, 80% AFUE, non-condensing, MCT ECM blower). **No `equipment_catalog` change needed — the model number was already right.**

**Real combustion-safety data sourced** (new migration `20260830010000_add_carrier_58sb0_furnace_combustion_specs.sql`), from the **58SB0B/58SB1B Installation Instructions (`58SB0B-01SI`, 46 pp.)** + **58SB0B Product Data (A190411)**, both read directly (PyMuPDF text extraction + high-res page renders for the graphical label). Covers both `58SB0090M17-14` (standard) and `58SB1090M17-14` (Low NOx) — the IOM is titled for both, so both rows are directly sourced, not inferred from a sibling:
- **Venting category: Category I** fan-assisted — SI p.4 verbatim ("This 4-way multipoise Category I fan-assisted furnace is CSA design-certified").
- **Clearances to combustibles** (SI Fig. 2 / PD "CLEARANCES", label drawing A220231, rating-plate label 336996-101 REV. F). Real platform difference vs. the Amana/Goodman furnaces already in this table: the label states **"clearance arrows do not change with furnace orientation"** — one set of values covers upflow/downflow/horizontal (no separate larger horizontal/alcove enclosure table). Inches: top/plenum **1**, back **0**, sides **0**, front **3**, bottom **0** (non-combustible floor only; combustible flooring only on accessory base KGASB0201ALL or a cased coil). Alcove/closet front clearance **18**. Separate front **service** clearance **24**. Vent connector to combustibles: single-wall **6**, Type B-1 **1**.
- **Natural-gas supply (inlet) pressure**: min **4.5** / max **13.6** in. w.c. (PD A190411 pp.4–5 GAS CONTROLS table; WhiteRodgers redundant single-stage valve, factory orifice #43, silicon-nitride HSI). SI p.14 also caps any inlet *test* pressure at 0.5 psig / 14 in. w.c.
- **Natural-gas manifold pressure**: **3.2–3.8** in. w.c. range at sea level (SI p.33 step 3 NOTE, verbatim). SI p.33 EXAMPLE 1 worked point: 3.7 in. w.c. with factory #43 orifice at 1000 Btu/cu-ft / 0.62 s.g. Stored as a 3.2/3.8 low/high range (single-stage valve, one physical regulator target expressed as a range); the 3.7 example point recorded in `source_document`, not invented as a single "nominal".

**Genuine disclosed gap — propane.** Neither the 58SB0B Product Data nor the 58SB0B Installation Instructions publishes a propane manifold pressure or a propane supply-pressure range. This platform ships for natural gas; propane operation requires the separate factory kit **AGAGC8NPS01B** (natural-to-propane, non-condensing 80% furnaces; listed in the PD A190411 accessory table), whose own Installation Instructions carry those values — could not be obtained from a primary source this pass (ManualsLib has the -01C revision but not as a readable PDF; shareddocs path not found). `manifold_pressure_propane_*` and `propane_supply_pressure_*` left **NULL** with an explicit disclosure in the migration (null/unconfirmed beats a guess), *not* carried over from a different manufacturer's platform. Whoever continues the Carrier sweep should source AGAGC8NPS01B once to fill propane fields for every Carrier 58-series 80% furnace platform at once.

**Migration design note**: this migration looks the two rows up by `(manufacturer, model_number, equipment_type)` and `raise exception` if not found, rather than hardcoding UUIDs (no live DB / Supabase MCP available this session to read real UUIDs; the `20260827260000` migration already uses the same tuple-lookup pattern for these exact rows).

**Verification**: SQL-only additive migration, no application code touched, table (`equipment_combustion_specs`) already exists from `20260827400000`. Not applied to a live DB this session (no connection available) — consistent with it being a pure data migration in an ongoing sweep; it will apply with the rest on the next `supabase db push`. Model strings verified exact against `20260827220000_carrier_full_current_lineup.sql`.
